...
#############################################
def create_word_subtitle_entries_for_intervals(word_list, intervals):
    """
    Given a list of word entries (each with "word" and "timing": {"start", "end"})
    and a list of intervals (each (start, end) in seconds defining the short clip),
    produce a list of subtitle entries for the final concatenated clip.
    
    The final clip is constructed by concatenating the intervals in order.
    For each interval, only the words whose timings fall completely within it
    are kept, and their timings are shifted relative to the final clip timeline.
    """
    entries = []
    current_offset = 0
    for interval in intervals:
        ...
        for word in word_list:
            word_start = word["timing"]["start"]
            word_end   = word["timing"]["end"]
            if word_start >= int_start and word_end <= int_end:
                # Adjust timings relative to this interval and then add current offset.
                adjusted_start = current_offset + (word_start - int_start)
                adjusted_end   = current_offset + (word_end - int_start)
                entries.append(((adjusted_start, adjusted_end), word["word"]))
        current_offset += (int_end - int_start)
    return entries

def extract_video_clip(video_file, time_segments, output_file, subtitle_text=None, word_list=None, use_word_by_word=False):
    """
    ...
    If use_word_by_word is True and word_list is provided, then overlay subtitles using the
    actual word-level timings (filtered to the intervals defined by time_segments). Otherwise,
    if subtitle_text (a full string) is provided, it will be split into chunks.
    ...
    try:
        ...
        with VideoFileClip(video_file) as clip:
            ...
            # Overlay subtitles
            if use_word_by_word and word_list is not None:
                # Use the provided word_list and the Gemini time segments as intervals.
                # Note: time_segments is a list of intervals (in seconds) relative to the original segment.
                # We'll assume these intervals, when concatenated, form the final clip.
                subtitle_entries = create_word_subtitle_entries_for_intervals(word_list, time_segments)
            elif subtitle_text:
                # Fallback: split the subtitle text into chunks of 4 words
